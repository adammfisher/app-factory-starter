...
| 4. Guardrails scale with what can't be undone | Code is reversible, so agents run free inside `app/`, `src/` and `infra/`. The worker cannot edit its rails, the PRD or the SPEC: `.claude/hooks/rails.mjs` blocks it, and `restoreRails()` plus the oracle hashes catch what the hook cannot see. Release cannot be undone, so `.eas/workflows/release.yml` has a `require-approval` job and `eas submit` is denied. Infrastructure is written by agents but never applied by them: `factory/infra.mjs` holds everything in `infra/` to a scale-to-zero policy kept as data (`factory/infra-policy.json`) in the gate, `runAgent()` strips every AWS credential from the agent's environment, and `.claude/settings.json` denies `terraform apply`, the `aws` CLI and state files. The PRD has a section for exactly this. |
...
| 6. Simple is a requirement | Five files run everything: `prd.mjs`, `loop.mjs`, `gate.mjs`, `agent.mjs`, `infra.mjs`. SPEC generation uses no model. `npm run factory:status` prints machinery lines against product lines. `npm run factory:selftest` runs it all for $0. |
